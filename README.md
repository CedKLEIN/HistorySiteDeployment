...
| `ADMIN_EMAIL` | `cedaout@hotmail.fr` | Email(s) admin qui reçoivent les messages de contact (`SiteConfiguration__AdminEmails__0`) **et** email du compte admin créé au démarrage (`SiteConfiguration__AdminSeed__Email`) |
...
> Le mot de passe Gmail et `JWT_SECRET` **ne doivent jamais** être commités : `.env` est dans `.gitignore`.
...
```ini
SMTP_PASSWORD=abcd efgh ijkl mnop    # envoi des messages de contact (mot de passe d'application Gmail)
...
ADMIN_EMAIL=cedaout@hotmail.fr       # email de ce compte admin (+ destinataire des messages de contact)
```
...
> En développement local, le compte admin **`cedaout@hotmail.fr` / `HistoryAdminDev1!`** est
